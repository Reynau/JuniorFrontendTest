...
function search () {
  ...
  searchProfile(search_value);
}

function searchProfile (user) {
  var url = "https://api.github.com/users/" + user;
  var ret = getProfile(url);
}
  ...
  xmlHttp.onreadystatechange = function() {
    if (xmlHttp.readyState == 4) {
      if (xmlHttp.status == 200)
        xmlRepos = getRepos(JSON.parse(xmlHttp.responseText));
      else searchNotFound();
    }
  };
...
function getRepos (profile_json) {
  var xmlHttp = new XMLHttpRequest();
  xmlHttp.onreadystatechange = function() {
    if (xmlHttp.readyState == 4) {
      if (xmlHttp.status == 200)
        processData(profile_json, JSON.parse(xmlHttp.responseText));
      else searchNotFound();
    }
  };
  xmlHttp.open("GET", profile_json.repos_url, true);
  xmlHttp.send(null);
}
